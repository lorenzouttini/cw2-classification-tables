...
def main():
    ...
    ucl_folders = sorted([f for f in os.listdir(base_path) if f.startswith("ucl")])
    print(f"Found UCL folders: {ucl_folders}")
    ...
    for folder_name in ucl_folders:
        ...
        if not (os.path.isdir(image_dir) and os.path.isdir(depth_dir)):
            print(f"  Skipping {scene_path} (missing image/ or depth/).")
            continue
        ...
        for img_file, gt_file in zip(image_files, depth_files):
            ...
            if match:
                ...
                print(f"  ⚠️ Warning: Could not extract frame number from filename '{img_file}', skipping.")
                continue
            ######################################################
            ...
            plot_depths_with_mask(
                ...
                os.path.join("figures/results/depths/ucl/fig", f"{folder_name}_{int(frame_num):03d}_depth_comparison_masked.png")    # FOR UCL
            )
            ...
            Image.fromarray((pred_depth_aligned * 1000).astype(np.uint16)).save(
                ...
                os.path.join(f"{scene_path}/depth_pred", f"{subfolder_name}_{int(frame_num):03d}_depth.png")    # FOR UCL
                )
    ...
    results_file = "figures/results/depths/ucl/results_depth.txt"
    ...
    plt.title("AbsRel Histogram (All UCL images)")
    plt.xlabel("AbsRel")
    ...
    plt.savefig("figures/results/depths/ucl/absrel_histogram.png")
    plt.close()
