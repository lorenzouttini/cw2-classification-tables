...
def process_depth(scene_path, output_dir, max_frames=100):
    ...
    for idx, img_name in enumerate(image_files):
        ...
        with torch.no_grad():
            # Note: The output field name may differ depending on the model.
            # Adjust "predicted_depth" if needed.
...
def main():
    ...
    print(f"Found MIT folders: {mit_folders}")

    for folder_name in mit_folders:
        ...
        for img_file, gt_file in zip(image_files, depth_files):
            ...
            plot_depths_with_mask(
                ...
                os.path.join("figures/results/depths", f"{folder_name}_{int(frame_index):04d}_depth_comparison_masked.png")
            )
    ...
    print("\n=== Averaged Depth Evaluation Metrics (all MIT folders) ===")
    print(f"AbsRel: {avg_absrel:.4f}")
    print(f"RMSE: {avg_rmse:.4f}")
    print(f"MAE: {avg_mae:.4f}")
    print(f"Log10: {avg_log10:.4f}")
    print(f"Total images processed: {len(absrel_list)}")
    print(f"Total folders processed: {len(mit_folders)}")

    # 10) Save results to a text file
    results_file = "figures/results/depths/results_depth.txt"
    with open(results_file, "w") as f:
        f.write("Averaged Depth Evaluation Metrics (all MIT folders):\n")
    ...
    plt.title("AbsRel Histogram (All MIT images)")
    plt.xlabel("AbsRel")
    plt.ylabel("Frequency")
    plt.tight_layout()
    plt.savefig("figures/results/depths/absrel_histogram.png")
    plt.close()
